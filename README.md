...
For Agent-mode streaming, there is also a Mastra-powered route at `src/app/api/tambo/route.ts` (requires Node 22+ and Azure OpenAI env vars: `AZURE_OPENAI_KEY` (preferred) / `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_DEPLOYMENT`). If both keys are set, `AZURE_OPENAI_KEY` takes precedence.
